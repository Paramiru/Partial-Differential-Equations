...
class PDE:
    def __init__(self, l=100, nstep=int(1e5)):
        self.l = l
        self.rng = np.random.default_rng()
        self.phi = self.initialise_phi()
        self.nstep = nstep
        self.mu = np.zeros((self.l, self.l))
        self.compute_mu()

    def initialise_phi(self):
        noise = (self.rng.random(size=(self.l, self.l)) - 0.5) * 0.1
        return np.ones((self.l, self.l)) * (self.rng.random() - 0.5) + noise
    ...
    def compute_mu(self, a=0.1, k=0.1):
        self.mu = -a * self.phi + a * self.phi**3 - k*self.laplacian(self.phi)
    ...
    def animate_cahn_hilliard(self):
        ...
        pd.DataFrame.from_dict(data).to_csv('free_energy_data.csv', index=False)
